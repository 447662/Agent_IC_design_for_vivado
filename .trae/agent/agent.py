...
""".format(user_input=user_input, skill_text=skill_text)
    ...
    def resolve_sync_fifo_vcd_path(self, output_dir="outputs"):
        return Path(output_dir) / "sync-fifo" / "sim" / "sync_fifo_trace.vcd"

    def collect_sync_fifo_vcd_analysis(self, output_dir="outputs", limit=20, waveform_backend="auto"):
    ...
    def analyze_sync_fifo_vcd(self, output_dir="outputs", limit=20, waveform_backend="auto"):
        try:
    ...
    def resolve_round_robin_arbiter_vcd_path(self, output_dir="outputs"):
        return Path(output_dir) / "round-robin-arbiter" / "sim" / "round_robin_arbiter_trace.vcd"

    def collect_round_robin_arbiter_vcd_analysis(self, output_dir="outputs", limit=20, waveform_backend="auto"):
    ...
    def analyze_round_robin_arbiter_vcd(self, output_dir="outputs", limit=20, waveform_backend="auto"):
        try:
...
"""
    ...
    def write_sync_fifo_sim_report(self, project_dir, vcd_path, wave_db_path, sim_result=None, project_result=None, limit=20):
        project_dir = Path(project_dir)
        reports_dir = project_dir / "reports"
        reports_dir.mkdir(parents=True, exist_ok=True)
        report_path = reports_dir / "sim_report.md"
...
""".replace("__DATA_WIDTH__", str(int(data_width))).replace("__ADDR_WIDTH__", str(int(addr_width)))
    ...
    def render_sync_fifo_tb(self, data_width=8, addr_width=4):
        return """`timescale 1ns/1ps
...
""".replace("__DATA_WIDTH__", str(int(data_width))).replace("__ADDR_WIDTH__", str(int(addr_width)))
    ...
    def render_sync_fifo_vivado_script(self):
        return """set script_dir [file dirname [file normalize [info script]]]
cd $script_dir
...
"""
    ...
    def render_sync_fifo_xsim_tcl(self):
        return """log_wave -r /
run all
quit
"""

    def render_sync_fifo_project_script(self):
        return self.render_vivado_tclstore_bootstrap() + """
set script_dir [file dirname [file normalize [info script]]]
cd $script_dir
set project_dir [file normalize [file join $script_dir .. vivado_project]]
...
"""
    ...
    def render_sync_fifo_open_project_gui_script(self):
        return """set script_dir [file dirname [file normalize [info script]]]
cd $script_dir
...
"""
    ...
    def render_sync_fifo_readme(self):
...
"""
    ...
    def write_sync_fifo_project(self, output_dir, data_width=8, addr_width=4):
    ...
    def render_round_robin_arbiter_rtl(self, requesters=4):
        return """`timescale 1ns/1ps
...
""".replace("__REQUESTERS__", str(int(requesters)))
    ...
    def render_round_robin_arbiter_tb(self, requesters=4):
        return """`timescale 1ns/1ps
...
""".replace("__REQUESTERS__", str(int(requesters)))
    ...
    def render_round_robin_arbiter_vivado_script(self):
        return """set script_dir [file dirname [file normalize [info script]]]
cd $script_dir
file mkdir ../vivado_project
set timestamp [clock format [clock seconds] -format "%Y%m%d_%H%M%S"]
...
"""
    ...
    def render_round_robin_arbiter_xsim_tcl(self):
        return """log_wave -r /
run all
quit
"""

    def render_round_robin_arbiter_project_script(self):
        return self.render_vivado_tclstore_bootstrap() + """
set script_dir [file dirname [file normalize [info script]]]
cd $script_dir
set project_dir [file normalize [file join $script_dir .. vivado_project]]
...
"""
    ...
    def render_round_robin_arbiter_open_project_gui_script(self):
        return """set script_dir [file dirname [file normalize [info script]]]
cd $script_dir
...
"""
    ...
    def render_round_robin_arbiter_readme(self):
...
"""
    ...
    def write_round_robin_arbiter_project(self, output_dir, requesters=4):
        project_dir = Path(output_dir) / "round-robin-arbiter"
        rtl_dir = project_dir / "rtl"
        tb_dir = project_dir / "tb"
        sim_dir = project_dir / "sim"
    ...
    def resolve_sync_fifo_wave_db(self, sim_dir):
    ...
    def resolve_round_robin_arbiter_wave_db(self, sim_dir):
        sim_dir = Path(sim_dir)
    ...
    def open_round_robin_arbiter_project_gui(self, project_dir):
        project_dir = Path(project_dir)
        sim_dir = project_dir / "sim"
    ...
    def open_sync_fifo_project_gui(self, project_dir):
        project_dir = Path(project_dir)
        sim_dir = project_dir / "sim"
...
""".replace("__WAVE_DB__", wave_db_name).replace(
    ...
    def run_sync_fifo_vivado_sim(self, output_dir="outputs", open_wave_gui=True, data_width=8, addr_width=4):
        project_dir = self.generate_rtl_project(
            "sync-fifo",
            output_dir,
            data_width=data_width,
    ...
    def run_round_robin_arbiter_vivado_sim(self, output_dir="outputs", open_wave_gui=True):
...
def main(argv=None):
    return run_cli(argv, create_agent)
