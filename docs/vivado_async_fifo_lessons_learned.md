...
## 问题 22：P3.14 真实 coverage 后必须刷新报告总览页

现象：真实运行 `python .trae/agent/agent.py --uvm-coverage async-fifo --coverage-threshold 1 --output-dir outputs` 后，`uvm_coverage_summary.md/html`、`uvm_coverage_xcrg/`、`xcrg_coverage.log` 和 `uvm_coverage_percent.txt` 都已生成，但 `reports/index.md/html` 仍停留在旧状态，没有出现 P3.13 的 coverage 入口。

原因：`write_async_fifo_reports_index()` 已经具备链接 coverage 产物的能力，但 `run_async_fifo_uvm_coverage()` 成功路径和失败路径只写 coverage 报告，没有在流程末尾刷新总览页。此前单元测试只覆盖“手动调用 index 生成函数”的场景，没有覆盖“真实 coverage runner 结束后总览页是否同步更新”。

修复：

- 在 `run_async_fifo_uvm_coverage()` 的成功路径写完 `uvm_coverage_summary.md/html` 后立即调用 `write_async_fifo_reports_index(project_dir)`。
- 在 Vivado coverage 失败路径写完失败摘要后也刷新 `reports/index.md/html`，确保失败时也能从统一入口看到诊断报告。
- 新增回归测试 `test_run_async_fifo_uvm_coverage_refreshes_reports_index`，复现“coverage PASS 但 index 不存在/未刷新”的问题，并断言 `uvm_coverage_summary.html`、官方 `xcrg` code/functional HTML、`xcrg_coverage.log` 和 `uvm_coverage_percent.txt` 同时出现在 Markdown 和 HTML 总览页。
- 真实验收重新运行 coverage 后确认 `outputs/async-fifo/reports/index.md/html` 已包含上述入口，随后通过 `--open-uvm-wave async-fifo --uvm-wave-kind coverage --output-dir outputs` 打开 Vivado GUI 的 `async_fifo_uvm_coverage.wdb`。

经验：

- “生成函数支持某个入口”不等于“真实工作流会刷新该入口”；所有阶段性产物写入后，都要检查用户实际打开的顶层入口是否同步更新。
- 报告总览页属于恢复项目上下文的第一入口，coverage/smoke/regression/wave 任一子流程生成新报告后，都应主动刷新 index。
- 真实工具验收后要回头检查最终用户可见 surface，不只检查底层产物是否存在。
