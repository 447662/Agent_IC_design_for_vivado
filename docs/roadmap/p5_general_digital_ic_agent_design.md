...
- P4.4：coverage history 和 Markdown/HTML 趋势报告已完成，schema 已包含 target、flow、toolchain 和 seed；post-P4 rotation 会把活动窗口之外的记录压缩归档。
- P4.5：失败运行通用归档已完成，固定保存 manifest、log、WDB、coverage DB、Tcl、目标配置、重跑脚本和波形脚本。
- P4.6：GUI 可见性自动验收已完成，统一检查 WDB、Scope、Object、Wave、Wave Config 和窗口像素指标，RTL/UVM 均复用 `wave_open_check` schema。
- P4.7：多 target 工程 dashboard 已完成，统一阶段状态、最近运行、最近失败、重跑命令、归档入口和七类报告卡片，并为尚未生成 dashboard 的目标提供顶层总览回退链接。
- P4 收尾复核已完成，缺失 manifest 不再生成断链；最终 `183 passed`、总覆盖率 `80.22%`，真实 dashboard 断链为 `0`，P4 基线已冻结。
- Post-P4 artifact/history rotation 已完成，target/environment manifest 与 coverage history 默认保留最近 `200` 条并压缩归档溢出记录。
