...
- 当前优先级：P4.0-P4.7、收尾复核和首项 post-P4 artifact/history rotation 已完成；下一优先项是 Windows/Vivado 自动验收。
...
- 新增 `.trae/agent/coverage_history.py`，以 JSONL 活动窗口保存 coverage 运行；post-P4 rotation 会把溢出记录压缩到 `coverage_history.archive.jsonl.gz`。
...
审计结论：`84/100`，可发布但有保留项。P4.0-P4.7 的本地质量门禁、真实报告入口、UTF-8、文档证据和历史 Vivado 验收均完整；本轮按用户要求未观测 GitHub Actions，且真实 Vivado GUI 自动验收仍依赖可交互 Windows 桌面，因此分数按发布审计规则封顶为 `84`。

收尾检查：

- P4.0-P4.7 每阶段均有唯一 TDD 证据，共 `31` 个显式命名 P4 测试。
- P4 路线图、README、后续待办和测试证据严格 UTF-8 解码通过；测试源码中的乱码 token 是用于回归检测的刻意样本。
- `13` 份 P4 相关 Markdown 文档的相对链接检查无断链。
- 初次真实 dashboard 扫描发现顶层总览存在 `3` 个缺失 manifest 断链：environment report、round-robin arbiter 和 sync FIFO。
- 修复后重新生成总览，扫描 `3` 个 dashboard、`116` 个相对链接，断链数为 `0`。
- Ruff 输出 `All checks passed!`；Mypy 输出 `Success: no issues found in 23 source files`。
- 最终全量回归 `183 passed in 25.62s`，总覆盖率 `80.22%`，`project_overview.py` 覆盖率 `90.9%`。
...
- GitHub Actions 本轮未观测；本地命令与 CI 配置已核对一致。
- Vivado GUI 像素与前台窗口验收仍需要具备桌面会话的 Windows runner。
- Artifact/history rotation 已在收尾后完成；归档恢复、合并和进一步分卷仍可作为后续运维增强。

详细证据见 `docs/testing/p4_closure_audit.md`。

## Post-P4：Artifact/History Rotation

状态：已完成。

- 新增 `.trae/agent/history_rotation.py`，统一校验活动记录限制、计算归档路径并追加 gzip JSONL。
- target/environment `artifacts.json` 默认保留最近 `200` 条运行，溢出记录写入 `artifacts.archive.jsonl.gz`。
- manifest 新增 `history.active_limit`、`history.archive_path` 和 `history.archived_runs`，保持 schema version `1` 向后兼容。
- `coverage_history.jsonl` 默认保留最近 `200` 条，溢出记录写入 `coverage_history.archive.jsonl.gz`，并由 `coverage_history.meta.json` 保存 rotation metadata。
- `coverage_trend.md/html` 展示活动记录数量、已归档记录数量和压缩归档入口。
- 传入 `None` 可关闭未来轮转；实现不会删除已有活动文件、metadata 或压缩归档。
- RED 为 `3 failed, 176 deselected`；GREEN 为 `3 passed, 176 deselected`；相关回归 `13 passed, 166 deselected`。
- 全量回归 `186 passed in 18.80s`，总覆盖率 `80.55%`，`history_rotation.py` 覆盖率 `90.9%`；Ruff 与 Mypy `24 source files` 均通过。
