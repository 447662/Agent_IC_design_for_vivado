...
| P4.5 | 已完成 | - | 失败 seed 自动归档 | 固定归档 log、WDB、coverage DB、Tcl、目标配置、manifest、重跑命令和波形命令 |
| P4.6 | 已完成 | - | GUI 可见性自动化增强 | 自动验证 WDB、Scope、Object、Wave、Wave Config 和窗口像素非空；保留运行时 JSON、PNG 与 Markdown/HTML 证据 |
| P4.7 | 已完成 | - | 报告体验升级 | 通用 target dashboard 统一 Spec/RTL/Simulation/UVM/Coverage/Wave/Lessons、最近运行、最近失败、重跑命令和归档入口 |

P4 收尾复核已完成：修复缺失 manifest 断链，最终 `183 passed`、总覆盖率 `80.22%`、真实 dashboard `116` 个相对链接且断链为 `0`。P4 基线已冻结。

Post-P4 artifact/history rotation 已完成：活动窗口默认 `200` 条，target/environment manifest 与 coverage history 的溢出记录均进入 gzip JSONL 归档；最终 `186 passed`、总覆盖率 `80.55%`。

## 已知技术债

- `agent.py` 仍承载较多 target 专用 RTL/TB/UVM flow；后续扩展时应继续按清晰边界迁移，但不为拆分而拆分。
- gzip JSONL 归档当前采用单文件持续追加；后续可增加按大小/日期分卷、归档恢复和合并工具。
...
- 文档统一使用 UTF-8；Windows PowerShell 5.1 读取无 BOM UTF-8 文件时应显式指定 `-Encoding UTF8`，避免把终端代码页显示问题误判为文件乱码。
- `outputs/` 当前用于本地真实 Vivado 验收，不应提交到仓库；如需要保留关键证据，应转写到 `docs/testing/` 或 `docs/roadmap/`。
- `.claude/settings.local.json` 是本地配置，不应提交。

## 推荐下一步

P4 基线已经冻结，建议按收益和依赖顺序收敛以下技术债：

1. **Windows/Vivado 自动验收**：增加具备桌面会话的 Windows runner，覆盖 Tcl 解析、WDB 打开和 GUI 可见性证据。
2. **Dashboard 链接 CI**：将真实报告相对链接扫描固化为自动测试，防止生成物入口回归。
3. **主类继续模块化**：按新增能力的自然边界继续拆分 `agent.py`，但不为拆分而拆分。
