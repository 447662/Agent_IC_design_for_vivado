...
1. **Artifact/history rotation**：为 target/environment `artifacts.json` 和 coverage history 增加保留数量、归档与压缩策略。
2. **Windows/Vivado 自动验收**：增加具备桌面会话的 Windows runner，覆盖 Tcl 解析、WDB 打开和 GUI 可见性证据。
