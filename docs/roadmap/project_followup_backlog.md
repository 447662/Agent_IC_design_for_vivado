...
1. **P4.1**：解析 xcrg HTML/log，提取低覆盖文件、实例、指标和来源。
2. **P4.2**：将低覆盖项映射到 `scenario_catalog`，输出可执行补测建议。
3. **P4.3**：为 Total/Line/Branch/Condition/Toggle/Functional coverage 增加独立 gate。
