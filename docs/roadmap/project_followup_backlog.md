...
截至 2026-07-09，项目已经完成从单点 async FIFO 到通用数字 IC Agent 的主流程雏形：
...
| 阶段 | 状态 | 建议优先级 | 目标 | 验收口径 |
|---|---|---:|---|---|
| P5.6 | 未完成 | P0 | P4 能力挂载点 | 定义通用 `coverage_metrics`、`scenario_catalog`、`artifact_manifest` 字段，并能在不同 target 上标记 PASS/SKIP/N/A |
...
| P5.9 | 未完成 | P1 | Adapter 拆分 | 将 Vivado、RWave/VCD、Report 逻辑从 `agent.py` 拆为 adapter，降低主文件继续膨胀的风险 |
...
- `agent.py` 已承载大量 target、Vivado、VCD、UVM、report 逻辑，继续扩展前建议优先做 P5.9 adapter 拆分。
- P5.4/P5.5 的 fallback catalog 当前仍在 Python 代码内，后续应逐步迁移到 `.trae/agent/targets/*.json`。
...
建议下一次开发从 P5.6 或 P5.7 开始：

1. **P5.6**：先定义统一 target metadata 扩展字段，让 P4 coverage、报告和场景建议有标准挂载点。
2. **P5.7**：再做 `--create-target`，把新增数字 IC target 的启动成本降下来。
3. **P5.9**：随后拆 adapter，避免继续在单个 `agent.py` 内堆叠所有工具逻辑。
