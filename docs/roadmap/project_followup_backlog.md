...
1. **P5.8**：让每次 flow 输出运行时 `artifacts.json`，把 P5.6 的预期 manifest 落到实际执行证据。
2. **P5.10**：生成中文环境预检报告，在进入 Vivado/RWave flow 前集中暴露工具、权限和 GUI 前置条件。
3. **P5.11**：在 manifest 与环境状态稳定后，构建多 target 顶层报告总览。
