...
| Full suite | First full run: 499 passed, 1 stale metadata assertion failed | Final local follow-up: 501 passed | All repository behavior and architecture tests pass after the terminology and CI portability fixes |
| Real PR CI | PR #1 initially failed on a Windows-path assertion under Linux and stale generated module counts | Cross-platform Vivado path regression: 4 passed; full local suite: 501 passed | Configured Windows Vivado paths normalize consistently on Windows and Linux runners |
...
| Full pytest with branch coverage and no cache provider | PASS, 501 tests |
| Coverage XML | Line 90.3%, branch 80.3%, configured gates passed |
...
- PR #1 recorded the expected fail-closed required context before the `vivado-integration` label was applied. The labeled real Vivado job is queued while the CI portability fix awaits explicit commit and push approval.
- The first Python quality PR run exposed the Linux Windows-path normalization regression and stale generated module counts; the local follow-up is green and needs one GitHub rerun after the fix is pushed.
