...
uv run --frozen mypy
uv run --frozen pytest tests --junitxml .tmp/pytest-results.xml --cov=.trae/agent --cov=src/digital_ic_agent --cov-report=term-missing --cov-report=xml:coverage.xml --basetemp .tmp-pytest -p no:cacheprovider
```
...
```text
src/digital_ic_agent/       # 可安装的公共包入口
.trae/agent/                # 核心运行时、目标插件、adapter 与 CLI
.trae/agent/targets/        # 内置目标元数据
...
公共包在已安装 wheel 中优先通过常规模块导入加载内置运行时；源码树兼容层只在加载期间临时暴露 legacy 模块目录，并在导入完成后恢复 `sys.path`。外部 target 插件在独立子进程中运行，受 manifest allowlist、输出根目录、读取边界和命令拒绝策略约束。
