...
- P3.11：Vivado 覆盖率百分比自动导出，coverage Tcl 会尝试生成 `reports/uvm_coverage_percent.txt`，runner 会自动解析 `Total Coverage` 并供 coverage gate 使用；若 Vivado 导出命令不可用，报告会保留失败原因并回退到元信息摘要。
