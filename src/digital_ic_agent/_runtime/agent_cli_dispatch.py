...
from digital_ic_agent._runtime.agent_cli import build_requirement, parse_seed_list
from digital_ic_agent._runtime.agent_errors import AgentError
...
def _required_mode_value(value: str | None) -> str:
    if value is None:
        raise ValueError("CLI mode requires a target or path value")
    return value
...
class CliArgs(Protocol):
    ...
    diagnostic: bool
    environment_report: bool
    generate_overview: bool
    coverage_closure: bool
    verify_waveform_samples: bool
    smoke_loop: bool
    sim_smoke: bool
    generate_rtl: str | None
    generate_spec: str | None
    generate_verification_plan: str | None
    sim_rtl: str | None
    regress_rtl: str | None
    uvm_smoke: str | None
    uvm_coverage: str | None
    uvm_random_regress: str | None
    analyze_rtl_vcd: str | None
    check_rtl: str | None
    open_wave: str | None
    open_uvm_wave: str | None
    analyze_vcd: str | None
    analyze_waveform: str | None
    output_dir: str
    requirement: Sequence[str]
    coverage_target: float
    coverage_threshold: float | None
    coverage_percent: float | None
    coverage_line_threshold: float | None
    coverage_branch_threshold: float | None
    coverage_condition_threshold: float | None
    coverage_toggle_threshold: float | None
    coverage_functional_threshold: float | None
    no_wave_gui: bool
    no_tool_check: bool
    uvm_seeds: str
    uvm_wave_kind: str
    vcd_condition: str | None
    vcd_show: str | None
    vcd_limit: int
    wave_backend: str
...
class CliAgent(Protocol):
    ...
    def write_environment_report(self, *, output_dir: PathValue) -> Report: ...

    def write_project_overview(self, *, output_dir: PathValue) -> Report: ...
    ...
    def write_waveform_sample_report(self, *, output_dir: PathValue) -> Report: ...
    ...
    def generate_rtl_project(self, target: str, output_dir: PathValue) -> Path: ...

    def write_target_design_spec(
        self,
        target: str,
        *,
        output_dir: PathValue,
    ...
    def analyze_sync_fifo_vcd(self, **kwargs: object) -> bool: ...

    def analyze_round_robin_arbiter_vcd(self, **kwargs: object) -> bool: ...
    ...
    def analyze_vcd(self, waveform_path: str, **kwargs: object) -> bool: ...

    def analyze_waveform(self, waveform_path: str, **kwargs: object) -> bool: ...
...
def _status_exit(report: Report) -> int:
    return 1 if report["status"] == "FAIL" else 0


def _print_paths(report: Report, labels: tuple[tuple[str, str], ...]) -> None:
    for label, key in labels:
        print("{}: {}".format(label, report[key]))


def _run_boolean_flow(
    label: str,
...
def _handle_listing_and_scaffold(args: CliArgs, agent: CliAgent) -> int | None:
    if args.list_skills:
        agent.list_skills()
        return 0
...
def _create_target(args: CliArgs, agent: CliAgent) -> int:
    try:
        description = " ".join(args.requirement).strip() or None
        scaffold = agent.create_target_scaffold(
            _required_mode_value(args.create_target),
...
def _handle_report_commands(args: CliArgs, agent: CliAgent) -> int | None:
...
def _write_environment_report(args: CliArgs, agent: CliAgent) -> int:
    try:
...
def _write_project_overview(args: CliArgs, agent: CliAgent) -> int:
    try:
...
def _write_coverage_closure(args: CliArgs, agent: CliAgent) -> int:
    try:
...
def _write_waveform_sample_report(args: CliArgs, agent: CliAgent) -> int:
    try:
...
def _handle_smoke_and_generation(args: CliArgs, agent: CliAgent) -> int | None:
    if args.smoke_loop:
...
def _generate_rtl_project(args: CliArgs, agent: CliAgent) -> int:
    try:
        project_dir = agent.generate_rtl_project(
            _required_mode_value(args.generate_rtl),
            args.output_dir,
        )
    except (OSError, ValueError) as exc:
...
def _write_design_spec(args: CliArgs, agent: CliAgent) -> int:
    try:
        report = agent.write_target_design_spec(
            _required_mode_value(args.generate_spec),
            output_dir=args.output_dir,
...
def _write_verification_plan(args: CliArgs, agent: CliAgent) -> int:
    try:
        report = agent.write_target_verification_plan(
            _required_mode_value(args.generate_verification_plan),
            output_dir=args.output_dir,
        )
    except (OSError, ValueError) as exc:
...
def _handle_rtl_and_uvm_flows(args: CliArgs, agent: CliAgent) -> int | None:
    if args.sim_rtl:
        return _run_boolean_flow(
            "RTL simulation",
            lambda: agent.run_rtl_sim(
                _required_mode_value(args.sim_rtl),
                output_dir=args.output_dir,
                open_wave_gui=not args.no_wave_gui,
            ),
            (OSError, RuntimeError, ValueError),
    ...
    if args.regress_rtl:
        return _run_boolean_flow(
            "RTL regression",
            lambda: agent.regress_rtl(
                _required_mode_value(args.regress_rtl),
                output_dir=args.output_dir,
                open_wave_gui=False,
            ),
            (OSError, ValueError),
    ...
    if args.uvm_smoke:
        return _run_boolean_flow(
            ...
            lambda: agent.run_uvm_smoke(
                _required_mode_value(args.uvm_smoke),
                output_dir=args.output_dir,
                open_wave_gui=not args.no_wave_gui,
            ),
            (OSError, RuntimeError, ValueError),
    ...
    if args.uvm_random_regress:
        return _run_boolean_flow(
            ...
            lambda: agent.run_uvm_random_regression(
                _required_mode_value(args.uvm_random_regress),
                output_dir=args.output_dir,
...
def _run_uvm_coverage(args: CliArgs, agent: CliAgent) -> int:
    try:
        coverage_thresholds = {
            metric: value
            for metric, value in (
        ...
        return 0 if agent.run_uvm_coverage(
            _required_mode_value(args.uvm_coverage),
            output_dir=args.output_dir,
...
def _handle_target_analysis_flows(args: CliArgs, agent: CliAgent) -> int | None:
    if args.analyze_rtl_vcd:
        ...
        return _run_boolean_flow(
            ...
            lambda: agent.run_target_flow(
                _required_mode_value(args.analyze_rtl_vcd),
                "analyze-rtl-vcd",
                output_dir=args.output_dir,
                limit=args.vcd_limit,
                waveform_backend=args.wave_backend,
    ...
    if args.check_rtl:
        return _run_boolean_flow(
            "RTL check",
            lambda: agent.run_target_flow(
                _required_mode_value(args.check_rtl),
                "check-rtl",
                output_dir=args.output_dir,
            ),
            (OSError, ValueError),
    ...
    if args.open_wave:
        return _run_boolean_flow(
            "RTL wave open",
            lambda: agent.open_rtl_wave(
                _required_mode_value(args.open_wave),
                output_dir=args.output_dir,
            ),
            (OSError, RuntimeError, ValueError),
        )
    if args.open_uvm_wave:
        return _run_boolean_flow(
            ...
            lambda: agent.open_uvm_wave(
                _required_mode_value(args.open_uvm_wave),
                output_dir=args.output_dir,
...
def _handle_waveform_analysis(args: CliArgs, agent: CliAgent) -> int | None:
    if args.analyze_vcd:
        return _run_boolean_flow(
            "Waveform analysis",
            lambda: agent.analyze_vcd(
                _required_mode_value(args.analyze_vcd),
    ...
    if args.analyze_waveform:
        return _run_boolean_flow(
            "Waveform analysis",
            lambda: agent.analyze_waveform(
                _required_mode_value(args.analyze_waveform),
                condition=args.vcd_condition,
                show=args.vcd_show,
                limit=args.vcd_limit,
                waveform_backend=args.wave_backend,
...
def _handle_default_workflow(args: CliArgs, agent: CliAgent) -> int:
...
def dispatch_cli_command(args: CliArgs, agent: CliAgent) -> int:
    handlers = (
