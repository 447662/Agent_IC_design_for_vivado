...
from digital_ic_agent._runtime.agent_contracts import AgentRequest, AgentRun, AgentRunStatus, ToolResult
from digital_ic_agent._runtime.capability_preflight import PreflightReport


class WorkflowSkill(Protocol):
    name: str
    action: str
...
class WorkflowAgent(Protocol):
    WARN: str
    OK: str
    loaded_skills: Mapping[str, WorkflowSkill]
    last_agent_run: AgentRun
    agent_execution_engine: WorkflowExecutionEngine

    def recommend_skills(self, user_input: str) -> Sequence[str]: ...
...
def build_workflow_tool_result_lines(
    agent: WorkflowAgent,
